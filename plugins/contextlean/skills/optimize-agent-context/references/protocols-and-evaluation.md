...
`jsonFileLineCitations` is an opt-in deterministic assertion for citation existence and structural validation. It parses one declared JSON array field, extracts one declared string field from every item, and accepts only exact `file:line` values whose portable relative file path is in the frozen allowlist, whose ASCII line number exists, and whose cited line contains visible text. Each source path is relative to the frozen tasks artifact and bound by SHA-256; absolute, parent, non-canonical, Windows-style, sensitive, missing, symlinked, changed, blank, and out-of-range inputs fail closed. The allowlist is evaluator input, not a permission system or proof that a source is trustworthy. This assertion does not establish citation semantic correctness; use task-specific sentinels or a separately authorized blind review for meaning.
